...
void CMFCApplication2Dlg::EnumLoad(TCHAR * load) {
	...
	do {
		...
		if (wsd.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY) {
			...
			mystrmd5.Format(L"%s\r\n", wsd.cFileName);
			this->myclear.SetSel(-1);
			this->myclear.ReplaceSel(mystrmd5);
////////////////////////////////////////////////////////////////
////读文件
//			try
//			{
...
//						//名字
//						const char* ch1 = mystr[0].c_str();
//						CStringA strPathA = CW2A(mystrmd5.GetBuffer(), CP_THREAD_ACP);
//						char*temp1 = strPathA.GetBuffer();
//						string cppstname1 = temp1;
//						string cppstname2 = ch1;
//						//md5
//						const char* ch2 = mystr[1].c_str();
//						const char* tempmd5 = md5FileValue(temp1);
//						string cppstrmd1 = this->md5;
//						string cppstrmd2 = ch2;
//						if (cppstname1 == cppstname2&&cppstrmd1 == cppstrmd2) {
//							MessageBox(L"不是毒！");
...
void CMFCApplication2Dlg::StartInject()
{
	HANDLE hMap;
...
BOOL CMFCApplication2Dlg::DoInjection(char *DllPath, HANDLE hProcess)
{
...
HANDLE CMFCApplication2Dlg::GetThePidOfTargetProcess(HWND hwnd)
{
	DWORD pid;
...
bool CMFCApplication2Dlg::StartInsert(char *title, char *winclass, char *dllpath)
{
	//开始注入
...
{
	if (StartInsert("任务管理器", NULL, "C:\\Users\\allenboy\\Desktop\\MFCLibrary1.dll") == false)
	{
